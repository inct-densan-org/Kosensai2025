  ...
  return (
    <div className={`px-12 py-0 ${width>height? "justify-center": ""}`}>
      ...
      <div style={open? width>height?{ paddingLeft:320 }:{ paddingTop:500 } : { paddingTop:100 }} >
        ...
        <MapPage base={map2.image} shops={map2.shops} labels={map2.labels} statics={map2.statics} currentId={index} w={width-(width>height?320:0)}/>
        <span className={`${width<height?"scroll-offset":""}`} id="map3"></span>
        <MapPage base={map3.image} shops={map3.shops} labels={map3.labels} statics={map3.statics} currentId={index} w={width-(width>height?320:0)}/>
        <span className={`${width<height?"scroll-offset":""}`} id="map4"></span>
        <MapPage base={map4.image} shops={map4.shops} labels={map4.labels} statics={map4.statics} currentId={index} w={width-(width>height?320:0)}/>
        <span className={`${width<height?"scroll-offset":""}`} id="map5"></span>
        <MapPage base={map5.image} shops={map5.shops} labels={map5.labels} statics={map5.statics} currentId={index} w={width-(width>height?320:0)}/>
      </div>
    </div>
  )}